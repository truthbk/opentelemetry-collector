...
func TestSpanEventSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestSpanEvent()
...
func TestSpanEventSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestSpanEventSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestSpanEventSlice().getOrig(), *dest.getOrig())
}
...
func TestSpanEventSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestSpanEventSlice().getOrig(), *es.getOrig())
...
func generateTestSpanEventSlice() SpanEventSlice {
	...
	*ms.getOrig() = internal.GenTestSpanEventPtrSlice()
	return ms
}
