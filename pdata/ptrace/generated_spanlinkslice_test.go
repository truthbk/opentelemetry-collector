...
func TestSpanLinkSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestSpanLink()
...
func TestSpanLinkSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestSpanLinkSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestSpanLinkSlice().getOrig(), *dest.getOrig())
}
...
func TestSpanLinkSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestSpanLinkSlice().getOrig(), *es.getOrig())
...
func generateTestSpanLinkSlice() SpanLinkSlice {
	ms := NewSpanLinkSlice()
	*ms.getOrig() = internal.GenTestSpanLinkPtrSlice()
	return ms
}
