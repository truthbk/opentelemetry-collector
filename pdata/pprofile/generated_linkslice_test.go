...
func TestLinkSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestLink()
...
func TestLinkSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestLinkSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestLinkSlice().getOrig(), *dest.getOrig())
}
...
func TestLinkSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestLinkSlice().getOrig(), *es.getOrig())
...
func generateTestLinkSlice() LinkSlice {
	ms := NewLinkSlice()
	*ms.getOrig() = internal.GenTestLinkPtrSlice()
	return ms
}
