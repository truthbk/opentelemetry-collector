...
func TestScopeProfilesSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestScopeProfiles()
...
func TestScopeProfilesSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestScopeProfilesSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestScopeProfilesSlice().getOrig(), *dest.getOrig())
}
...
func TestScopeProfilesSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestScopeProfilesSlice().getOrig(), *es.getOrig())
...
func generateTestScopeProfilesSlice() ScopeProfilesSlice {
	...
	*ms.getOrig() = internal.GenTestScopeProfilesPtrSlice()
	return ms
}
