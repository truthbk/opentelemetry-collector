...
func TestFunctionSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestFunction()
...
func TestFunctionSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestFunctionSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestFunctionSlice().getOrig(), *dest.getOrig())
}
...
func TestFunctionSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestFunctionSlice().getOrig(), *es.getOrig())
...
func generateTestFunctionSlice() FunctionSlice {
	ms := NewFunctionSlice()
	*ms.getOrig() = internal.GenTestFunctionPtrSlice()
	return ms
}
