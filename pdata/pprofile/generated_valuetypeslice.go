...
func (es ValueTypeSlice) EnsureCapacity(newCap int) {
	...
	newOrig := make([]*internal.ValueType, len(*es.getOrig()), newCap)
...
func (es ValueTypeSlice) AppendEmpty() ValueType {
	es.getState().AssertMutable()
	*es.getOrig() = append(*es.getOrig(), internal.NewValueType())
...
func (es ValueTypeSlice) RemoveIf(f func(ValueType) bool) {
	...
	for i := 0; i < len(*es.getOrig()); i++ {
		if f(es.At(i)) {
			internal.DeleteValueType((*es.getOrig())[i], true)
			(*es.getOrig())[i] = nil
...
func (es ValueTypeSlice) CopyTo(dest ValueTypeSlice) {
	...
	*dest.getOrig() = internal.CopyValueTypePtrSlice(*dest.getOrig(), *es.getOrig())
}
...
func (ms ValueTypeSlice) getOrig() *[]*internal.ValueType {
	return ms.orig
}
