...
func TestLineSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestLine()
...
func TestLineSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestLineSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestLineSlice().getOrig(), *dest.getOrig())
}
...
func TestLineSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestLineSlice().getOrig(), *es.getOrig())
...
func generateTestLineSlice() LineSlice {
	ms := NewLineSlice()
	*ms.getOrig() = internal.GenTestLinePtrSlice()
	return ms
}
