...
func (es StackSlice) At(i int) Stack {
	return newStack((*es.getOrig())[i], es.getState())
}
...
func (es StackSlice) EnsureCapacity(newCap int) {
	...
	newOrig := make([]*internal.Stack, len(*es.getOrig()), newCap)
...
func (es StackSlice) AppendEmpty() Stack {
	es.getState().AssertMutable()
	*es.getOrig() = append(*es.getOrig(), internal.NewStack())
...
func (es StackSlice) RemoveIf(f func(Stack) bool) {
	...
	for i := 0; i < len(*es.getOrig()); i++ {
		if f(es.At(i)) {
			internal.DeleteStack((*es.getOrig())[i], true)
			(*es.getOrig())[i] = nil
...
func (es StackSlice) CopyTo(dest StackSlice) {
	...
	*dest.getOrig() = internal.CopyStackPtrSlice(*dest.getOrig(), *es.getOrig())
}
...
func (ms StackSlice) getOrig() *[]*internal.Stack {
	return ms.orig
}

func (ms StackSlice) getState() *internal.State {
	return ms.state
}
