...
func (es KeyValueAndUnitSlice) At(i int) KeyValueAndUnit {
	return newKeyValueAndUnit((*es.getOrig())[i], es.getState())
}
...
func (es KeyValueAndUnitSlice) EnsureCapacity(newCap int) {
	...
	newOrig := make([]*internal.KeyValueAndUnit, len(*es.getOrig()), newCap)
...
func (es KeyValueAndUnitSlice) AppendEmpty() KeyValueAndUnit {
	es.getState().AssertMutable()
	*es.getOrig() = append(*es.getOrig(), internal.NewKeyValueAndUnit())
...
func (es KeyValueAndUnitSlice) RemoveIf(f func(KeyValueAndUnit) bool) {
	...
	for i := 0; i < len(*es.getOrig()); i++ {
		if f(es.At(i)) {
			internal.DeleteKeyValueAndUnit((*es.getOrig())[i], true)
			(*es.getOrig())[i] = nil
...
func (es KeyValueAndUnitSlice) CopyTo(dest KeyValueAndUnitSlice) {
	...
	*dest.getOrig() = internal.CopyKeyValueAndUnitPtrSlice(*dest.getOrig(), *es.getOrig())
}
...
func (ms KeyValueAndUnitSlice) getOrig() *[]*internal.KeyValueAndUnit {
	return ms.orig
}

func (ms KeyValueAndUnitSlice) getState() *internal.State {
	return ms.state
}
