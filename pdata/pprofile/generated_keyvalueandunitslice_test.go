...
func TestKeyValueAndUnitSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestKeyValueAndUnit()
...
func TestKeyValueAndUnitSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestKeyValueAndUnitSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestKeyValueAndUnitSlice().getOrig(), *dest.getOrig())
}
...
func TestKeyValueAndUnitSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestKeyValueAndUnitSlice().getOrig(), *es.getOrig())
...
func generateTestKeyValueAndUnitSlice() KeyValueAndUnitSlice {
	...
	*ms.getOrig() = internal.GenTestKeyValueAndUnitPtrSlice()
	return ms
}
