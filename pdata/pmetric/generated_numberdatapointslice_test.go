...
func TestNumberDataPointSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestNumberDataPoint()
...
func TestNumberDataPointSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestNumberDataPointSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestNumberDataPointSlice().getOrig(), *dest.getOrig())
}
...
func TestNumberDataPointSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestNumberDataPointSlice().getOrig(), *es.getOrig())
...
func generateTestNumberDataPointSlice() NumberDataPointSlice {
	...
	*ms.getOrig() = internal.GenTestNumberDataPointPtrSlice()
	return ms
}
