...
func (es SummaryDataPointSlice) At(i int) SummaryDataPoint {
	return newSummaryDataPoint((*es.getOrig())[i], es.getState())
}
...
func (es SummaryDataPointSlice) EnsureCapacity(newCap int) {
	...
	newOrig := make([]*internal.SummaryDataPoint, len(*es.getOrig()), newCap)
...
func (es SummaryDataPointSlice) AppendEmpty() SummaryDataPoint {
	es.getState().AssertMutable()
	*es.getOrig() = append(*es.getOrig(), internal.NewSummaryDataPoint())
...
func (es SummaryDataPointSlice) RemoveIf(f func(SummaryDataPoint) bool) {
	...
	for i := 0; i < len(*es.getOrig()); i++ {
		if f(es.At(i)) {
			internal.DeleteSummaryDataPoint((*es.getOrig())[i], true)
			(*es.getOrig())[i] = nil
...
func (es SummaryDataPointSlice) CopyTo(dest SummaryDataPointSlice) {
	...
	*dest.getOrig() = internal.CopySummaryDataPointPtrSlice(*dest.getOrig(), *es.getOrig())
}
...
func (ms SummaryDataPointSlice) getOrig() *[]*internal.SummaryDataPoint {
	return ms.orig
}

func (ms SummaryDataPointSlice) getState() *internal.State {
	return ms.state
}
