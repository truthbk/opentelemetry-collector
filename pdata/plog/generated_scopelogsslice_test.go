...
func TestScopeLogsSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestScopeLogs()
...
func TestScopeLogsSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestScopeLogsSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestScopeLogsSlice().getOrig(), *dest.getOrig())
}
...
func TestScopeLogsSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestScopeLogsSlice().getOrig(), *es.getOrig())
...
func generateTestScopeLogsSlice() ScopeLogsSlice {
	...
	*ms.getOrig() = internal.GenTestScopeLogsPtrSlice()
	return ms
}
