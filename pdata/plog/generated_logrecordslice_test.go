...
func TestLogRecordSlice(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		(*es.getOrig())[i] = internal.GenTestLogRecord()
...
func TestLogRecordSlice_CopyTo(t *testing.T) {
	...
	src.CopyTo(dest)
	assert.Equal(t, *generateTestLogRecordSlice().getOrig(), *dest.getOrig())
	dest.CopyTo(dest)
	assert.Equal(t, *generateTestLogRecordSlice().getOrig(), *dest.getOrig())
}
...
func TestLogRecordSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, *generateTestLogRecordSlice().getOrig(), *es.getOrig())
...
func generateTestLogRecordSlice() LogRecordSlice {
	...
	*ms.getOrig() = internal.GenTestLogRecordPtrSlice()
	return ms
}
